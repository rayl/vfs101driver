...
static int validity_cycle4 (struct vfs_dev *dev)
{
	...
	_(  Peek(dev, 0xF0, 0x1F, 0x00, 0x00, 0x04));
	_(  Peek(dev, 0xF4, 0x1F, 0x00, 0x00, 0x04));
	_(  Peek(dev, 0xF8, 0x1F, 0x00, 0x00, 0x04));
	_(  Peek(dev, 0xFC, 0x1F, 0x00, 0x00, 0x04));
	_(  GetParam(dev, 0x2e));
	_(  GetVersion(dev));
	_(  GetParam(dev, 0x28));
	_(  GetParam(dev, 0x14));
...
static int validity_cycle3 (struct vfs_dev *dev)
{
	...
	_(  Peek(dev, 0x2C, 0x50, 0xFF, 0x00, 0x02));
	_(  Peek(dev, 0x2E, 0x50, 0xFF, 0x00, 0x02));
	_(  Poke(dev, 0xF6, 0x05, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x01));
	_(  Peek(dev, 0x3E, 0x50, 0xFF, 0x00, 0x01));
	_(  Poke(dev, 0x3E, 0x50, 0xFF, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01));
	_(  Peek(dev, 0x02, 0x98, 0xFF, 0x00, 0x01));
	_(  Peek(dev, 0x00, 0x98, 0xFF, 0x00, 0x01));
	_(  Peek(dev, 0x06, 0x98, 0xFF, 0x00, 0x01));
	_(  Poke(dev, 0x06, 0x98, 0xFF, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01));
	_(  GetPrint(dev, 100, data109));
	_(  LoadImage(dev));
	_(  Poke(dev, 0xF6, 0x05, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01));
	_(  Peek(dev, 0x3E, 0x50, 0xFF, 0x00, 0x01));
	_(  Poke(dev, 0x3E, 0x50, 0xFF, 0x00, 0x10, 0x00, 0x00, 0x00, 0x01));
	_(  Peek(dev, 0x02, 0x98, 0xFF, 0x00, 0x01));
	_(  Peek(dev, 0x00, 0x98, 0xFF, 0x00, 0x01));
	_(  Peek(dev, 0x06, 0x98, 0xFF, 0x00, 0x01));
	_(  GetPrint(dev, 100, data116));
	_(  LoadImage(dev));
	_(  Peek(dev, 0x38, 0x50, 0xFF, 0x00, 0x01));
	_(  Peek(dev, 0x0E, 0x50, 0xFF, 0x00, 0x02));
	_(  Peek(dev, 0x32, 0x50, 0xFF, 0x00, 0x01));
	_(  Poke(dev, 0x32, 0x50, 0xFF, 0x00, 0x12, 0x00, 0x00, 0x00, 0x01));
	_(  Poke(dev, 0x0E, 0x50, 0xFF, 0x00, 0x00, 0x40, 0x00, 0x00, 0x02));
	_(  Poke(dev, 0x38, 0x50, 0xFF, 0x00, 0x0F, 0x00, 0x00, 0x00, 0x01));
	_(  SetParam(dev, 0x0062, 0x0000));
	...
	_(  Peek(dev, 0x00, 0x98, 0xFF, 0x00, 0x01));
	_(  Peek(dev, 0x06, 0x98, 0xFF, 0x00, 0x01));
	_(  Poke(dev, 0x38, 0x50, 0xFF, 0x00, 0x0E, 0x00, 0x00, 0x00, 0x01));
	_(  GetPrint(dev, 10, data135));
	_(  LoadImage(dev));
	_(  Poke(dev, 0x38, 0x50, 0xFF, 0x00, 0x0D, 0x00, 0x00, 0x00, 0x01));
	_(  GetPrint(dev, 10, data137));
	_(  LoadImage(dev));
	_(  Poke(dev, 0x38, 0x50, 0xFF, 0x00, 0x0C, 0x00, 0x00, 0x00, 0x01));
	_(  GetPrint(dev, 10, data139));
	_(  LoadImage(dev));
	_(  Poke(dev, 0x38, 0x50, 0xFF, 0x00, 0x0B, 0x00, 0x00, 0x00, 0x01));
	_(  GetPrint(dev, 10, data141));
	_(  LoadImage(dev));
	_(  Poke(dev, 0x38, 0x50, 0xFF, 0x00, 0x0A, 0x00, 0x00, 0x00, 0x01));
	_(  GetPrint(dev, 10, data143));
	_(  LoadImage(dev));
	_(  Poke(dev, 0x38, 0x50, 0xFF, 0x00, 0x09, 0x00, 0x00, 0x00, 0x01));
	_(  GetPrint(dev, 10, data145));
	_(  LoadImage(dev));
	_(  Poke(dev, 0x38, 0x50, 0xFF, 0x00, 0x08, 0x00, 0x00, 0x00, 0x01));
	_(  GetPrint(dev, 10, data147));
	_(  LoadImage(dev));
	_(  Poke(dev, 0x38, 0x50, 0xFF, 0x00, 0x07, 0x00, 0x00, 0x00, 0x01));
	...
	_(  Poke(dev, 0x38, 0x50, 0xFF, 0x00, 0x14, 0x00, 0x00, 0x00, 0x01));
	_(  Poke(dev, 0x0E, 0x50, 0xFF, 0x00, 0xB4, 0x21, 0x00, 0x00, 0x02));
	_(  Poke(dev, 0x32, 0x50, 0xFF, 0x00, 0x31, 0x00, 0x00, 0x00, 0x01));
